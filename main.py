    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Gerador e checker de nitro by isabella & nycolli")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Gerador e checker de nitro by isabella & nycolli\a',
        ...
        self.slowType("Made by: isabella & nycolli", .02)
        ...
        if USE_WEBHOOK:
            ...
            if webhook is not None:
                DiscordWebhook(  # Let the user know it has started logging the ids
                        url=url,
                        content=f"```Quando um codigo valido for encontrado vai aparecer aqui para ser resgatado ( isabella.py#6971 & nycolli.js#9700
 <3 ) ```"
                    ).execute()
        ...
        for s in c:  # Loop over the amount of codes to check
            ...
            if os.name == "nt":  # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Complexo dos Crias Gerador & Checker Beta - {len(valid)} Valido | {invalid} Invalido - by isabella & nycolli")  # Change the title
                print("")
            else:  # If it is a unix system
                # Change the title
                print(
                    f'\33]0;Completo dos Crias Gerador & Checker Beta - {len(valid)} Valido | {invalid} Invalido - by isabella & nycolli\a', end='', flush=True)
        ...
        if response.status_code == 200:  # If the responce went through
            ...
            if notify is not None:  # If a webhook has been added
                DiscordWebhook(  # Send the message to discord letting the user know there has been a valid nitro code
                    url=url,
                    content=f"Nitro valido caralho! @everyone \n{nitro}"
                ).execute()
